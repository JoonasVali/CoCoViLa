...
/**
 * Loads stored schemes from .syn files. Instances of this scheme loader should
 * be safely reusable as long as it is not used in parallel.
 * 
 * This loader tries to solve minor mismatches between the package description
 * and saved scheme. For example, new fields are quietly accepted. On the other
 * hand, warnings are generated in case of missing fields of classes. The user
 * should be notified about any warnings generated.
 * 
...
public class SchemeLoader implements DiagnosticsCollector.Diagnosable {
	...
	/**
	 * Indicates whether the scheme was successfully loaded. The method returns
	 * the value returned by the load() method.
	 * 
	...
	/**
	 * Reads in the scheme description from a .syn file. The setPackage() method
	 * must be called with a non-null argument before attempting to load any
	 * schemes. The results can be asked with get*() methods after this method
	 * returns.
	 * 
	 * @param file
	 *          scheme file
	...
	public boolean load(File file) {
		...
		if (file.isFile() && file.length() == 0L) {
			collector.collectDiagnostic("The file " + file.getName() + " is empty!",
			    true);
	...
	/**
	 * Preparations for loading a scheme. This method makes sure that a package is
	 * set and the parser is available.
	 * 
	...
	/**
	 * Parses the scheme XML. The method checkInitLoad() must be called at least
	 * once before calling this method.
	 * 
	 * @param input
	 *          the stream that produces the scheme XML to parse
	 * @return true on success, false otherwise
	 */
	...
	/**
	 * Collects messages. Some messages may be fatal.
	 * 
	 * @param msg
	 * @param isFatal
	 */
	...
	public ObjectList getObjectList() {
		if (handler != null) return handler.getObjects();
		return null;
	}

	public ConnectionList getConnectionList() {
		if (handler != null) return handler.getConnections();
	...
	class PackageHandler extends DefaultHandler {
		...
		public void error(SAXParseException spe) throws SAXParseException {
			String msg = "Parsing error, line " + spe.getLineNumber() + ", uri "
			    + spe.getSystemId();
			db.p(msg);
		...
		public void endDocument() {
			if (superClass != null && objects.getByName(superClass) == null) {
				collectDiagnostic("Superclass " + superClass + " not found.");
				superClass = null;
			}
		}
			...
			if (element.equals(OBJECT) || element.equals(RELOBJECT)) {
				...
				if (!type.equals(vPackage.getName())) {
					...
					if (res != JOptionPane.YES_OPTION) {
						...
						throw new SchemeLoaderException("Scheme was built with package \""
						    + type + "\", not \"" + vPackage.getName(), false);
					}
				}
			}
				...
				if (!pclass.hasField(name, type)) {
					collectDiagnostic("The class " + obj.getName() + " has saved field "
					    + name + "(" + type + ")" + " = " + value
				...
				if (INPUT.equals(nature))
					cf.setInput(true);
				else if (GOAL.equals(nature)) cf.setGoal(true);
				...
				if (beginPort == null || endPort == null) {
					collectDiagnostic("Discarding connection " + obj1 + "." + port1
					    + " = " + obj2 + "." + port2
	...
	public Scheme getScheme(ISchemeContainer canvas) {
		...
		if (handler != null) scheme.setSpecText(handler.getSchemeExtSpec());
		return scheme;
	}
}
